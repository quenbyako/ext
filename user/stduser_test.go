// Copyright (c) 2020-2024 Richard Cooper
//
// This file is a part of quenbyako/ext package.
// See https://github.com/quenbyako/ext/blob/master/LICENSE for details

package user

import (
	"os"
	"reflect"
	"testing"

	"github.com/quenbyako/ext/slices"
)

func TestResolveHomedir(t *testing.T) {
	t.Parallel()

	p := &TestProvider{
		CurrentID: 10,
		Users: map[int]*User{
			10: {HomeDir: "/nfs_home/someuser"},
		},
	}

	for _, tt := range []struct {
		name     string
		in       string
		provider Provider
		want     string
		wantErr  errorAssertionFunc
	}{
		{"", "/", p, "/", nil},
		{"", "~/", p, "/nfs_home/someuser/", nil},
		{"", "~", p, "/nfs_home/someuser", nil},
		{"", "not_home/~", p, "not_home/~", nil},
	} {
		tt := tt
		tt.wantErr = noErrAsDefault(tt.wantErr)

		t.Run(tt.name, func(t *testing.T) {
			got, err := ResolveHomedir(p, tt.in)
			tt.wantErr(t, err)

			assertEqual(t, tt.want, got)
		})
	}

	t.Run("os_specific", func(t *testing.T) {
		const path = "~/.local/yourapp/config.yml"

		realDir, err := os.UserHomeDir()
		noError(t, err)

		wantPath := realDir + "/.local/yourapp/config.yml"

		got, err := ResolveHomedir(OSProvider{}, path)
		noError(t, err)
		assertEqual(t, wantPath, got)
	})

}

func TestCurrent(t *testing.T) {
	u, err := Current()
	if err != nil {
		t.Fatalf("Current: %v (got %#v)", err, u)
	}
	if u.HomeDir == "" {
		t.Errorf("didn't get a HomeDir")
	}
	if u.Username == "" {
		t.Errorf("didn't get a username")
	}
}

func BenchmarkCurrent(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Current() //nolint:errcheck // in benchmark makes no sense
	}
}

func compare(t *testing.T, want, got *User) {
	if want.Uid != got.Uid {
		t.Errorf("got Uid=%q; want %q", got.Uid, want.Uid)
	}
	if want.Username != got.Username {
		t.Errorf("got Username=%q; want %q", got.Username, want.Username)
	}
	if want.Name != got.Name {
		t.Errorf("got Name=%q; want %q", got.Name, want.Name)
	}
	if want.HomeDir != got.HomeDir {
		t.Errorf("got HomeDir=%q; want %q", got.HomeDir, want.HomeDir)
	}
	if want.Gid != got.Gid {
		t.Errorf("got Gid=%q; want %q", got.Gid, want.Gid)
	}
}

func TestLookup(t *testing.T) {
	want, err := Current()
	if err != nil {
		t.Fatalf("Current: %v", err)
	}
	// TODO: Lookup() has a fast path that calls Current() and returns if the
	// usernames match, so this test does not exercise very much. It would be
	// good to try and test finding a different user than the current user.
	got, err := Lookup(want.Username)
	if err != nil {
		t.Fatalf("Lookup: %v", err)
	}
	compare(t, want, got)
}

func TestLookupId(t *testing.T) {
	want, err := Current()
	if err != nil {
		t.Fatalf("Current: %v", err)
	}
	got, err := LookupId(want.Uid)
	if err != nil {
		t.Fatalf("LookupId: %v", err)
	}
	compare(t, want, got)
}

func TestLookupGroup(t *testing.T) {
	user, err := Current()
	if err != nil {
		t.Fatalf("Current(): %v", err)
	}

	g1, err := LookupGroupId(user.Gid)
	if err != nil {
		// NOTE(rsc): Maybe the group isn't defined. That's fine.
		// On my OS X laptop, rsc logs in with group 5000 even
		// though there's no name for group 5000. Such is Unix.
		t.Logf("LookupGroupId(%q): %v", user.Gid, err)
		return
	}
	if g1.Gid != user.Gid {
		t.Errorf("LookupGroupId(%q).Gid = %s; want %s", user.Gid, g1.Gid, user.Gid)
	}

	g2, err := LookupGroup(g1.Name)
	if err != nil {
		t.Fatalf("LookupGroup(%q): %v", g1.Name, err)
	}
	if g1.Gid != g2.Gid || g1.Name != g2.Name {
		t.Errorf("LookupGroup(%q) = %+v; want %+v", g1.Name, g2, g1)
	}
}

func TestGroupIds(t *testing.T) {
	user, err := Current()
	if err != nil {
		t.Fatalf("Current(): %v", err)
	}
	gids, err := user.GroupIds()
	if err != nil {
		t.Fatalf("%+v.GroupIds(): %v", user, err)
	}
	if !slices.Contains(gids, user.Gid) {
		t.Errorf("%+v.GroupIds() = %v; does not contain user GID %s", user, gids, user.Gid)
	}
}

func noErrAsDefault(errfunc errorAssertionFunc) errorAssertionFunc {
	if errfunc == nil {
		return noError
	}
	return errfunc
}

type errorAssertionFunc func(*testing.T, error)

func noError(t *testing.T, err error) {
	if err != nil {
		t.Log("expected no error")
	}
}

func assertEqual[T any](t *testing.T, want, got T) {
	if !reflect.DeepEqual(want, got) {
		t.Logf("want: %#v, got %#v", want, got)
		t.FailNow()
	}
}
